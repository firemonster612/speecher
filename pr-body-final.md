...
The install section also comes first on the page, and every shortcut control stays hidden until the install has run: a shortcut bound to the pre-install path (in particular the manual command on desktops without a shortcut portal) would break the moment the install moves the image. The manual command is generated only afterwards, against `~/.local/bin/speecher`.
...
![ydotool enable locked](https://raw.githubusercontent.com/firemonster612/speecher/evidence/pr84-linux-setup/ydotool-enable-locked.png)

Unlocked after a passed test; enabling still takes the click:

![ydotool enable unlocked](https://raw.githubusercontent.com/firemonster612/speecher/evidence/pr84-linux-setup/ydotool-enable-unlocked.png)
...
![transcription page with score](https://raw.githubusercontent.com/firemonster612/speecher/evidence/pr84-linux-setup/setup-transcription.png)

![refinement page with score](https://raw.githubusercontent.com/firemonster612/speecher/evidence/pr84-linux-setup/setup-refinement.png)

## The gate, before and after

Before installing: only the install step, with Next disabled and Skip setup gone.

![global shortcut page before install](https://raw.githubusercontent.com/firemonster612/speecher/evidence/pr84-linux-setup/setup-global-shortcut.png)

After installing: button reads Installed, Next and Skip return, and the shortcut instructions appear with a command that goes through `~/.local/bin/speecher`.

![global shortcut page after install](https://raw.githubusercontent.com/firemonster612/speecher/evidence/pr84-linux-setup/setup-global-shortcut-installed.png)
...
- New committed E2E rig at `tests/e2e/linux-setup` (same pattern as `linux-update`): builds the AppImage, runs it on a fresh profile under kwin_wayland in bwrap, and drives the whole assistant over AT-SPI. Final run passed all twelve assertions: score lines on both provider pages, Skip hidden and shortcut controls absent while the install is pending, Next held, the move into `~/Applications`, command link, menu entry and icon present, Skip and Next returning, the shortcut command appearing only afterwards through the installed path, and setup completing only after the install.
